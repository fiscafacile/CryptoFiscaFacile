...
type CsvTXExSupercharger struct {
	Time        time.Time
	Currency    string
...
func (cdc *CryptoCom) ParseCSVExSupercharger(reader io.Reader) (err error) {
	csvReader := csv.NewReader(reader)
	records, err := csvReader.ReadAll()
	if err == nil {
		for _, r := range records {
			if r[0] != "create_time_utc" {
				tx := CsvTXExSupercharger{}
				...
				tx.Description = r[3]
				cdc.CsvTXsExSupercharger = append(cdc.CsvTXsExSupercharger, tx)
